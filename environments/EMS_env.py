...
def  default_rwd_fun(s, a, s_next):
    ...
    delta_abs_error = np.abs(current_error) - np.abs(next_error) 
    
    delta_error = delta_abs_error if delta_abs_error > 0 else 2*delta_abs_error 
    reward = -next_error**2#100*delta_error/s[0] # Normalize delta error to be a fraction of the daily demand 
    ...
    reward += 6*e_balance if e_balance < 0 else e_balance

    return np.array([reward], dtype=np.float32)

class EMS_env2(DiscreteCustomEnv):
    """
    Environment for the Energy Management System
    """
    ...
    def __init__(self, rwd_function = None, render: bool = True):
        ...
        self.action_space = spaces.Discrete(4) # 16
        super().__init__(np.array([0.0, 25., 50., 75., 100.], dtype=np.float32))
        #super().__init__(np.array(np.meshgrid(self.Q_p_levels, self.Irr_levels), dtype=np.float32).T.reshape(-1, 2))
        #self.action_values = np.array(np.meshgrid(self.Irr_levels, self.Q_p_levels), dtype=np.float32).T.reshape(-1, 2)
        #self.action_values = np.array([0.0, 25., 50., 75., 100.], dtype=np.float32)
